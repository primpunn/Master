...
class OmniMiniProj:
	...
	def __init__(self):
		...
		self.joint_state_sub = rospy.Subscriber("/phantom/pose", geometry_msgs.msg.PoseStamped, self.get_joint_states, queue_size=1)
	...
	#get_joint_states extracts the joint angles from the JointState message  (angles, agular velocity, ...), which is
	#provided by the 'joint_state_sub' subscriber.
	def get_joint_states(self,data):
		try:
			q_sensors = data.position
			#q_sensors = data.geometry_msgs
